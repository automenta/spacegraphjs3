...
export class EnhancedIntegrationDemo {
  ...
  private initializeScene(): void {
    ...
    this.spaceGraph = new SpaceGraph('#enhanced-demo-container', {
      data: { nodes: [], edges: [] },
      layout: { type: 'force-directed', charge: -100, linkDistance: 10, linkStrength: 1 },
  ...
  private setupEventHandlers(): void {
    ...
    this.interactionSystem.on('hoverstart', (object: THREE.Object3D) => {
      this.hudSystem.setStatus(`Hovering over ${object.userData.label || 'object'}`, 2000);
    ...
    this.interactionSystem.on('doubleclick', (object: THREE.Object3D) => {
      ...
      this.hudSystem.showNotification(`Framed ${object.userData.label || 'object'}`, 'info', 2000);
    });
    ...
    this.interactionSystem.on('select', (object: THREE.Object3D) => {
      ...
      this.hudSystem.setStatus(`Selected ${object.userData.label || 'object'}`, 2000);
    });
  ...
  private setupHUDButts(): void {
    ...
    if (resetCameraBtn) {
      resetCameraBtn.querySelector('button')?.addEventListener('click', () => {
        ...
        this.hudSystem.showNotification('Camera reset to default position', 'info', 2000);
      });
    }
    
    const toggleWireframeBtn = this.hudSystem.getElement('toggle-wireframe');
    if (toggleWireframeBtn) {
      toggleWireframeBtn.querySelector('button')?.addEventListener('click', () => {
        this.toggleWireframe();
        this.hudSystem.showNotification('Wireframe mode toggled', 'info', 2000);
      });
    }
    ...
    if (toggleStatsBtn) {
      toggleStatsBtn.querySelector('button')?.addEventListener('click', () => {
        this.toggleStats();
        this.hudSystem.showNotification('Statistics display toggled', 'info', 2000);
      });
    }
  }
    ...
    switch (event.key.toLowerCase()) {
      ...
      case 'f':
        ...
        if (selectedObjects.size > 0) {
          const objects = Array.from(selectedObjects as Set<string>).map((id: string) =>
            this.scene.getObjectByProperty('uuid', id)
          ).filter((obj): obj is THREE.Object3D => obj !== undefined);
          if (objects.length > 0) {
            this.cameraControls.frameObjects(objects);
            this.hudSystem.showNotification(`Focused on ${objects.length} objects`, 'info', 2000);
          }
        }
        break;
      case 'a':
...
export function createEnhancedIntegrationDemo(container: HTMLElement): EnhancedIntegrationDemo {
