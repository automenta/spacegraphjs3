...
export class EdgeRenderer {
  ...
  private edgeStates: Map<
    string,
    { hovered: boolean; selected: boolean; editing: boolean }
  > = new Map();
  ...
  constructor(scene: THREE.Scene, state: Store<Spec>) {
    ...
    this.lineSegments = new THREE.LineSegments(
      this.lineSegmentGeometry,
      this.lineSegmentMaterial
    );
  ...
  private createEdgeGeometry(
    ...
    switch (edge.type) {
      case 'curved':
        return this.createCurvedEdgeGeometry(
          sourcePos,
          targetPos,
          edge.curvature || 0.3
        );
      case 'dashed':
        return this.createDashedEdgeGeometry(
          sourcePos,
          targetPos,
          edge.dashSize || 0.5,
          edge.gapSize || 0.3
  ...
  private createEdgeMaterial(
    edge: EdgeSpec,
    state: { hovered: boolean; selected: boolean; editing?: boolean }
  ): THREE.Material {
    ...
    const isSourceSelected =
      sourceNode &&
      this.state.interaction?.selectedElementIds?.includes(sourceNode.id);
    const isTargetSelected =
      targetNode &&
      this.state.interaction?.selectedElementIds?.includes(targetNode.id);
  ...
  private updateEdgeVisuals(
    ...
    if (line.material) {
      ...
      this.animateEdgeMaterial(
        edge.id,
        line.material as THREE.Material | THREE.Material[],
        targetColor,
        targetOpacity
      );
  ...
  public setEdgeHover(edgeId: string, hovered: boolean): void {
    ...
    if (edge) {
      const sourceNode = this.state.data.nodes.find(
        (n) => n.id === edge.source
      );
      const targetNode = this.state.data.nodes.find(
        (n) => n.id === edge.target
      );
  ...
  private showEdgeEditHandles(
    ...
    const controlPoint = midPoint
      ...
      .add(
        perpendicular.multiplyScalar(
          curvature * sourcePos.distanceTo(targetPos) * 0.5
        )
      );
