...
export class VisualFeedbackSystem {
   ...
   */
  private createEnhancedGlowEffect(
    ...
    this.activeAnimations.set(`${object.uuid}-glow`, () =>
      stopAnimation.stop()
    );
   ...
   */
  private createEnhancedPulseEffect(
    object: THREE.Object3D,
    config: FeedbackConfig
  ): THREE.Mesh | null {
    const geometry = this.getBoundingGeometry(object);
    ...
    this.activeAnimations.set(`${object.uuid}-pulse`, () =>
      stopAnimation.stop()
    );
   ...
   */
  private createEnhancedParticleEffect(
    object: THREE.Object3D,
    config: FeedbackConfig
  ): THREE.Points | null {
    ...
    const particleColor = new THREE.Color(
      config.color || this.getDefaultColor(config.type)
    );
    ...
    this.activeAnimations.set(`${object.uuid}-particles`, () =>
      stopAnimation()
    );
  ...
  private animateScale(
    ...
    this.activeAnimations.set(`${object.uuid}-scale`, () =>
      stopAnimation.stop()
    );
  }
  ...
  private animateColorChange(
    ...
    this.activeAnimations.set(`${object.uuid}-color`, () =>
      stopAnimation.stop()
    );
  }
  ...
  public clearFeedback(objectId: string): void {
    ...
    const animationKeys = Array.from(this.activeAnimations.keys()).filter(
      (key) => key.startsWith(objectId)
    );
   ...
   */
  private getEmissionPattern(type: FeedbackType): (
    index: number,
    total: number
  ) => {
   ...
   */
  private animateEnhancedColorChange(
    object: THREE.Object3D,
    targetColor: string,
    config: FeedbackConfig
  ): void {
  ...
  private animateBounce(
    ...
    this.activeAnimations.set(`${object.uuid}-bounce`, () =>
      stopAnimation.stop()
    );
  }
  ...
  private animateWave(object: THREE.Object3D, duration: number): void {
    ...
    const wave = () => {
      ...
      const scaleY =
        originalScale.y * (1 + Math.cos(time * 1.1) * waveIntensity);
      const scaleZ =
        originalScale.z * (1 + Math.sin(time * 0.9) * waveIntensity);
  ...
  private animateRotation(
    ...
    this.activeAnimations.set(`${object.uuid}-rotation`, () =>
      stopAnimation.stop()
    );
  }
