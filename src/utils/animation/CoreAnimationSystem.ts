...
import { ValueInterpolationUtils } from './ValueInterpolationUtils';
...
export class UnifiedAnimationSystem extends BaseUtilitySystem {
  ...
  public async bounce(
    ...
    const originalValue = ValueInterpolationUtils.cloneValue(target[property]);

    const offset1 = ValueInterpolationUtils.offsetValue(originalValue, intensity);
    await this.tween(target, property, originalValue, offset1, {
      duration: 100,
    });

    const offset2 = ValueInterpolationUtils.offsetValue(originalValue, -intensity * 0.5);
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const animation = animate({
          from: ValueInterpolationUtils.extractValue(from),
          to: ValueInterpolationUtils.extractValue(to),
          duration,
          ease: easingFunction,
          onUpdate: (value) => {
            ValueInterpolationUtils.setTargetValue(
              target,
              property,
              ValueInterpolationUtils.interpolateValue(from, to, value)
            );
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const animate = (currentTime: number) => {
          ...
          ValueInterpolationUtils.setTargetValue(target, property, currentValue);
  ...
  private async executeSpring(
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const from = ValueInterpolationUtils.cloneValue(target[property]);
  ...
  private async executeDecay(
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const to = ValueInterpolationUtils.multiplyValue(from, 0.1); // Reduce to 10% of original
  ...
  private evaluateKeyframes(keyframes: Keyframe[], progress: number): any {
    ...
    return ValueInterpolationUtils.interpolateValue(
      startKeyframe.value,
      endKeyframe.value,
      easedProgress
    );
